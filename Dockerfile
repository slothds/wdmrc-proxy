...
RUN     apk add --no-cache --virtual .install-dep ca-certificates curl && \
        ...
        WDMRC_VERS=$(curl -sL ${WDMRC_REPO}/latest | sed -rn 's|.*<a.*WebDAVCloudMailRu-([.[:digit:]]*)-dotNetCore.*|\1|p;') \
        && \
        WDMRC_DNET=$(curl -sL ${WDMRC_REPO}/latest | sed -rn 's|.*<a.*WebDAVCloudMailRu-.*(dotNetCore[[:digit:]]*)\.zip.*|\1|p;') \
        && \
        echo "Download WebDavMailRuCloud v${WDMRC_VERS}-${WDMRC_DNET}" && \
        curl -kfSL ${WDMRC_REPO}/download/${WDMRC_VERS}/WebDAVCloudMailRu-${WDMRC_VERS}-${WDMRC_DNET}.zip \
